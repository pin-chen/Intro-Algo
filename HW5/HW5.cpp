...
using namespace std;
int edge[65536][2], node[100000], cur;
...
int main(){
	...
	while(t--){
		...
		for(i = 0, j = 0; i < n; i++){
			...
			for(k = 0; ; k++){
				//cout << b << " " << k << " " << edge[(b>>k)&HASH_MASK][0] <<" " << (edge[(b>>k)&HASH_MASK][0]==b) <<'\n';
		...
		for(i = 0; ; i++){
			for(k = 0; k < 9;k++){
				//cout << node[i] << " " << edge[(node[i]>>k)&HASH_MASK][0] << '\n';
		...
		for(i = I; i > 0; i--){
			...
			a = cur;
			for(k = 0; k < 9;k++){
				//cout << cur << " " << edge[(cur>>k)&HASH_MASK][0] << '\n';
		...
		for(i = 0; i < j; i++){
			...
			for(k = 0; k < 9;k++){
				if(edge[(node[i]>>k)&HASH_MASK][0]==node[i]) break;
				if(edge[(node[i]>>k)&HASH_MASK][0]==0)break;
				//cout << "F";
			}
