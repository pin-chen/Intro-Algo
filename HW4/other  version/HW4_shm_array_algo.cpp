...
void merge1(int32_t* initial, const int start, const int mid, const int end){
    ...
    inplace_merge(initial+start, initial+mid+1, initial+end+1, std::greater<int32_t>());
}
...
void merge_sort(int32_t* arr){
    for(int size = small_case; size < n; size += size){
	    ...
	    for(i = 0; i < n - 2 * size + 1; i += size + size){
	        merge1(arr, i, i + size - 1, i + 2 * size - 1);
	    }
	    if(n - i + 1 > size)   merge1(arr, i, i + size - 1, n - 1);
	    else                   merge1(arr, i, n - 1, n - 1);
	}
}
